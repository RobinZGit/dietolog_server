...
public interface NutrientsRepository  extends JpaRepository<Nutrients, Long>{
  ...
  @Query(value = "select _id, name, units, dailyrate \n"+
                 ", min_dailyrate, max_dailyrate, case n.units when 'г' then 1000 when 'мг' then 1 when 'мкг' then 0.001 when 'кКал' then 100000 else 1 end koeff_to_miligr, 0 val, 0 excluded \n" +
                //хинт тормозит..
