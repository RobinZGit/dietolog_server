...
public interface ProductsRepository extends JpaRepository<Products, Long> {
  ...
  @Query(value = "select    \n"+ 
                 ...
                 " , row_number() over(order by COALESCE(ii.value,0) desc,p._id) rownumber, p._id _id, name, lowercase, 0 val, 0 isrecommended, 0 isnotrecommended, 0 excluded \n"+ 
                 " from products p \n"+
  ...
  @Query(value = "select string_agg(name||' - '||cast(value as text)||' '||units||' на 100гр. ('||perc1on100gr||'% сут.нормы)',chr(13))  from (select n.name,i.value, i.perc1on100gr, n.units from info i left join nutrients n on n._id=i.nutrient where i.product=:productId order by COALESCE(i.perc1on100gr,'') desc limit 20) ZZ  \n"
                 , nativeQuery = true)
